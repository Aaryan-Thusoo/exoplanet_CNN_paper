...
There are several limitations of the work in this paper which are important to address. The first is the use of confirmed exoplanets and eclipsing binaries for the training and testing. These inputs are already known and thus have the most well fitting data the Kepler dataset has. Future missions and other inputs do not have known labels and therefore would be harder to classify in future models.

For the purpose of increasing the number of usable instances, the data was split into 100-day chunks. Rather than keeping each KepID as one singular instance, each became several inputs. Doing so increases the risks of creating chunks with poor signals while others within the same object contain better information. One instance can be predicted as separate classes which needs to be monitored when making splits.
